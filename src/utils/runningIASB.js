...
async function runningIASB(ditaFiles) {
    const iasbJsonPath = path.resolve(__dirname, '../../../public/XSLT_Files/IASB.sef.json');
    ...
    try {
        execSync(
            `xslt3 -t -xsl:${path.resolve(__dirname, '../../public/XSLT_Files/IASB.xsl')} -export:${iasbJsonPath} -nogo`
        );
    } catch (error) {
    ...
    if (!fs.existsSync(iasbJsonPath)) {
        throw new Error('XSLT transformation failed to create the required JSON files.');
    }
